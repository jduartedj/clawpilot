    ...
    for (const file of files) {
        ...
        if (file.includeContent && totalChars < maxImportedTotalChars) {
            ...
            const content = raw.length > maxImportedFileChars
                ? `${raw.slice(0, maxImportedFileChars)}\n\n[...truncated by PilotClaw agent import: file exceeds ${maxImportedFileChars} characters...]`
                : raw;
...
<!--
Generated by PilotClaw from OpenClaw agent "${id}".
PilotClaw import schema: ${importSchemaVersion}
Sources:
...
async function syncAgents() {
    ...
    for (const agent of agents) {
        ...
        try {
            ...
            targetHasCurrentSchema = targetPreview.includes(`PilotClaw import schema: ${importSchemaVersion}`);
        } catch (err) {
            if (err?.code !== "ENOENT") throw err;
        }
